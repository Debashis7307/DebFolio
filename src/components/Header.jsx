...
const Header = () => {
  ...
  return (
    <header className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 ${isSticky ? 'py-2' : 'py-4'}`}>
      <div className={`container mx-auto  transition-all duration-300 ${isSticky ? 'px-3 md:px-3' : 'px-0'}`}>
        <motion.div 
          className={`flex justify-between  items-center transition-all duration-300 ${isSticky ? 'bg-gradient-to-r from-blue-900/30 to-gray-900/30 backdrop-blur-md border-b border-primary/20 rounded-xl shadow-2xl shadow-[-8px_-8px_15px_rgba(255,255,255,0.1),10px_10px_15px_rgba(0,0,0,0.8)] shadow-primary/20 px-6 py-2' : ''}`}
        ...
        >
          ...
          >
            <img src="src\assets\images\photo5.png" alt="logo" className="w-10 h-10 rounded-full mr-2" />
        ...
        <motion.div
          ...
          className={`fixed top-0 right-0 w-55 h-screen bg-dark/95 backdrop-blur-lg shadow-2xl md:hidden z-40 pt-20 px-6 ${isMenuOpen ? 'shadow-[0_0_30px_rgba(0,238,255,0.3)]' : ''}`}
          style={{
